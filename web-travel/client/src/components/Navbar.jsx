...
const Navbar = ({ isOtherPage }) => {
	...
	const LinkItem = ({ to, href, children }) => {
		return isOtherPage ? (
			<Link
				to={to}
				className="block py-2 px-3 text-white font-semibold hover:text-blue-700 rounded md:bg-transparent md:p-0"
			>
				{children}
			</Link>
		) : (
		...
		<>
			...
			>
				<div className="mx-auto w-full flex items-center justify-between px-7 md:px-12 py-4">
					...
					>
						<ul
							className={`bg-neutral-800 md:bg-transparent flex flex-col items-center font-medium p-4 md:p-0 border border-gray-100 md:space-x-8 rtl:space-x-reverse md:flex-row md:mt-0 md:border-0 text-slate-100 transition-all duration-700 rounded-lg ${
								visible ? "translate-y-0" : "-translate-y-full"
							}`}
						>
							<li
